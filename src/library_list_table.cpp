...
{
  //----------------------------------------------------------------------------
  library_list_table::library_list_table(QWidget *parent):
    QTableWidget(0,1,parent)
  {
    QStringList l_h_header_list;
    l_h_header_list << tr("Name") ;    
  ...
  //----------------------------------------------------------------------------
  void library_list_table::add_library(const std::string &p_name)
  {
      setRowCount(rowCount()+1);
      setItem(rowCount()-1,0,new simple_table_cell(QString(p_name.c_str())));
