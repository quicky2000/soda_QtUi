...
namespace soda_QtUi
{
    ...
    m_soda_thread(NULL)
  {
    ...
    configuration_widget * l_configuration_widget = new configuration_widget();
    l_scroll_area->setWidget(l_configuration_widget);
  ...
  void main_window::create_actions(void)
  {
    ...
    m_launch_thread_action = new QAction(tr("&Open"),this);
    m_launch_thread_action->setShortcut(tr("Ctrl+O"));
    m_launch_thread_action->setStatusTip(tr("Open configuration file")); 
    connect(m_launch_thread_action,SIGNAL(triggered()),this,SLOT(treat_open_event()));
